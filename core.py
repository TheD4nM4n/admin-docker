...
class AdminBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        for module in listdir('./admin/modules'):
            if module.endswith('.py') and module != "__init__.py":
                self.load_extension(f'admin.modules.{module[:-3]}')

        print(f"[INFO] {len(self.extensions)} modules loaded!")
    ...
    async def on_guild_join(self, guild: Guild) -> None:
        ...
        if str(guild.id) not in config.config:
            ...
            # Writes the new config to disk
            config.save_data(config.config, config.CONFIG_LOCATION)
...
async def reload(ctx: commands.Context, module_name=None):
    if module_name is None:
        ...
        for module_title in listdir("admin/modules"):
...
async def modules(ctx: commands.Context):
    ...
    inactive_modules = ''
    for admin_module in listdir("admin/modules"):
...
async def enable(ctx: commands.Context, module_name):
    if module_name.lower() not in admin.extensions:
        if f"{module_name.lower()}.py" in listdir("admin/modules"):
